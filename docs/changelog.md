...
| **B1 — Supabase Migration System** | Production-safe, migrations-only database foundation: `supabase/migrations/001_initial_game_schema.sql` — additive & idempotent (pgcrypto, `gen_random_uuid()`, `create … if not exists`, `create or replace function/trigger`, `drop policy if exists`). 6 tables (`profiles`, `missions`, `student_progress`, `mission_attempts` [immutable replay log], `xp_events`, `certificates`) with uuid PKs, FKs, indexes, jsonb payloads, shared `set_updated_at()` trigger. RLS enabled on every table with secure-by-default starter policies (public read-only catalog; anon insert/select only — no update/delete, so no cross-row tampering; append-only ledgers; `auth.uid()` owner policies ready for future auth). Docs rewritten: [SUPABASE_SCHEMA.md](./SUPABASE_SCHEMA.md) + new [SUPABASE_MIGRATION_SETUP.md](./SUPABASE_MIGRATION_SETUP.md). No app code touched; supersedes the B0 mirror tables as the target production schema. |
| **B2 — Full Mission Sync** | `002_seed_missions.sql` seeds `mission-001/002/003` catalog rows so FK lookups resolve. `003_server_triggers.sql` adds two SECURITY DEFINER triggers: `trg_mission_attempt_upsert` (AFTER INSERT on `mission_attempts` → upserts `student_progress` + increments `profiles.missions_completed` on first completion) and `trg_xp_event_rollup` (AFTER INSERT on `xp_events` → increments `profiles.xp_total` + `trust_score`). `backendSync.js` gains `getMissionId()` (cached catalog lookup) + `cloudCompleteMissionAttempt()` (anon INSERT into `mission_attempts` with full scorecard payload). `script.js` wires the new call in `notifyAssignmentComplete` (after `trackXpEvent`). Result: all 5 tables populate on mission completion; server triggers maintain `student_progress` and profile totals without any client UPDATE permission. |
