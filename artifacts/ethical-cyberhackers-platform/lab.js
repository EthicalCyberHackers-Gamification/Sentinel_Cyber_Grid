...
/* ------------------------------------------------------------------ *
 * HOST BRIDGE (graduation)
 * ------------------------------------------------------------------ *
 * In the prototype this module toggled the prototype's own `opsCenter` /
 * `socConsole` screens directly. In the shipping game the host (script.js) owns
 * screen management and persistence, so the lab calls injected hooks instead:
 *   onOpen()              — host hides its screens before the lab is shown
 *   onReturn()            — host restores the Operations Center (+ repaint)
 *   onComplete(missionId) — host records real completion (flag + XP + save)
 * All hooks are optional; absent hooks are no-ops (keeps the module standalone).
 */
let LAB_HOOKS = {};
...
function labSubmitReport() {
  ...
  // Graduation — record REAL completion in the host (set the mission-complete
  // flag, award XP, persist, fire analytics). Best-effort: a throwing host must
  // never break the lab's own scorecard flow.
 ...
 * EXPORTS (graduation)
 * ------------------------------------------------------------------ *
 * The prototype published its entry points on `window` because oc.js and lab.js
 * were separate, unlinked ES modules. In the shipping game the host imports this
 * module directly, so we export the public surface and let the host call
 * initLab() during its own boot (instead of auto-initialising on DOMContentLoaded,
 * which could race the host's DOM setup).
 */
const LAB_MISSION_IDS = Object.keys(LAB_MISSIONS);
export { openLab, labInit as initLab, LAB_MISSION_IDS };
